...
class ViewDetails extends Component {
    ...
    render() {
        const { navigation } = this.props;
        const login_id = navigation.getParam('Login_Id');
        const user_name = navigation.getParam('User_Name');
        const mobile_no = navigation.getParam('Mobile_No');
        const user_category = navigation.getParam('User_Category');
        const date_time = navigation.getParam('Date_Time');
        const user_comments = navigation.getParam('User_Comments');
        const agent_comments = navigation.getParam('Agent_Comments');
        const user_status = navigation.getParam('User_Status');
        const unpaid_amount = navigation.getParam('Unpaid_Amount');
            ...
            <View>
                ...
                <View style={{ flexDirection: 'row', height: 55, backgroundColor: Constants.Colors.whiteColor, position: 'relative', shadowColor: "#000", shadowOffset: { width: 0, height: 12 }, shadowOpacity: 0.58, shadowRadius: 16.00, elevation: 24 }}>
                    <TouchableOpacity style={{ width: 45, paddingLeft: 20, justifyContent: 'center' }}
                        onPress={() => this.props.navigation.navigate('AllUsersQueryForUserOnly')}>
                    ...
                    <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
                        <View style={{ width: '70%', height: '70%', backgroundColor: '#ededed', alignItems: 'center', justifyContent: 'center', borderRadius: 20 }}>
                            <Text style={{ fontSize: 18, color: '#00517b', fontWeight: 'bold' }}>View Details</Text>
                        </View>
                    </View>
                </View>
                {/*  Header Bar End */}
                ...
                <View style={styles.container}>
                    ...
                    <View style={styles.innerContainer}>
                        ...
                        <View style={styles.btnContainer}>
                            <Text style={{ fontSize: 12 , fontWeight: 'bold' , marginRight: 20}}>Id</Text>
                            <Text>{login_id}</Text>
                        </View>
                        <View style={styles.btnContainer}>
                            <Text style={{ fontSize: 12 , fontWeight: 'bold' , marginRight: 20}}>Name</Text>
                            <Text>{user_name}</Text>
                        </View>
                        <View style={styles.btnContainer}>
                            <Text style={{ fontSize: 12 , fontWeight: 'bold' , marginRight: 20}}>Mobile no</Text>
                            <Text>{mobile_no}</Text>
                        </View>
                        <View style={styles.btnContainer}>
                            <Text style={{ fontSize: 12 , fontWeight: 'bold' , marginRight: 20}}>Category</Text>
                            <Text>{user_category}</Text>
                        </View>
                        <View style={styles.btnContainer}>
                            <Text style={{ fontSize: 12 , fontWeight: 'bold' , marginRight: 20}}>DateTime</Text>
                            <Text>{moment(date_time).format("LLL")}</Text>
                        </View>
                        <View style={styles.btnContainer}>
                            <Text style={{ fontSize: 12 , fontWeight: 'bold' , marginRight: 20}}>Comments</Text>
                            <Text>{user_comments}</Text>
                        </View>
                        <View style={styles.btnContainer}>
                            <Text style={{ fontSize: 12 , fontWeight: 'bold' , marginRight: 20}}>Agent Comments</Text>
                            <Text>{agent_comments}</Text>
                        </View>
                        <View style={styles.btnContainer}>
                            <Text style={{ fontSize: 12 , fontWeight: 'bold' , marginRight: 20}}>Status</Text>
                            <Text>{user_status}</Text>
                        </View>
                        <View style={styles.btnContainer}>
                            <Text style={{ fontSize: 12 , fontWeight: 'bold' , marginRight: 20}}>Unpaid Amount</Text>
                            <Text>{unpaid_amount}</Text>
                        </View>
                    </View>
                </View>
            </View>
...
export default connect(mapStateToProps, mapsDispatchToProps)(ViewDetails);