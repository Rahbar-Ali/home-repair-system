...
class AllAgents extends Component {
    constructor(props) {
        ...
        console.log("Construtor Runs All AGents..");
    }
    state = {
        listOfAllUsers: [],
        listOfAllAgents: [],
    ...
    componentDidMount() {
        this.onRefresh();
        console.log("Component runs... agent");
    ...
    shouldComponentUpdate(nextProps, nextState){
        ...
        if(this.state.listOfAllUsers != nextState.listOfAllUsers){
            return true;
        }
        else{
            return false;
    ...
    // shouldComponentUpdate( nextProps , nextState ){
    //     console.log('=============================================> shouldComponentUpdate runs');
    //     if(this.state.listOfAllUsers != nextState.listOfAllUsers || this.state.listOfAllAgents != nextState.listOfAllAgents ){
    //         this.getUserInfoList();
    //         this.getAllAgentList();
    //         return true;
    //     }
    ...
    getUserInfoList = () => {
        ...
        let url = GLOABAL_PATH.API_URL + Constants.ApiController.Login + Constants.ApiActionLogin.GetAllActiveAgents;
    ...
    makeUsersFunction = () => {
        ...
        let url = GLOABAL_PATH.API_URL + Constants.ApiController.Login + Constants.ApiActionLogin.MakeUsers;
                ...
                <View style={{ flexDirection: 'row', height: 55, backgroundColor: Constants.Colors.whiteColor, shadowColor: "#000", shadowOffset: { width: 0, height: 12 }, shadowOpacity: 0.58, shadowRadius: 16.00, elevation: 24 }}>
                    ...
                    <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
                        <View style={{ width: '70%', height: '70%', backgroundColor: '#ededed', alignItems: 'center', justifyContent: 'center', borderRadius: 20 }}>
                            <Text style={{ fontSize: 18, color: '#00517b', fontWeight: 'bold' }}>ALL AGENTS</Text>
                        </View>
                    </View>
                </View>
                {/*  Header Bar End */}
                                        ...
                                        <Right>
                                            <View onTouchStart={() => this.setState({ onSelectOptionMenu: item.Id })}>
                                                <OptionsMenu customButton={optionIcon} buttonStyle={{ width: 32, height: 8, margin: 7.5, resizeMode: "contain" }}
                                                    ...
                                                    options={["Make User", "Cancel"]}
                                                    actions={[this.makeUsersFunction]}
                                                />
                                            </View>
...
export default connect(null, mapsDispatchToProps)(AllAgents);